...
contract Genesis {
    ...
    function createProject(
        ...
        project.status = statusEnum.OPEN; // Явно укажем начальный статус
        ...
        projectCount++; // инкрементируем после использования текущего projectCount
        return true;
    }
    ...
    function backProject(uint id) public payable returns (bool) {
        require(msg.value > 0, "Ether must be greater than zero");
    ...
    function requestRefund(uint id) public returns (bool) {
        // ИСПРАВЛЕНИЕ: проверяем, что проект либо уже REVERTED, либо DELETED.
        require(
            projects[id].status == statusEnum.REVERTED || 
            projects[id].status == statusEnum.DELETED,
        ...
        // Если статус уже REVERTED или DELETED, просто возвращаем деньги
        performRefund(id);
        return true;
    }
